<template>
  <div class="flex flex-col items-center space-y-5 w-2/3 mx-auto" >
    ...
    <h5 class="font-bold text-lg">ورود به حساب کاربری</h5>
    <input type="text" v-model="loginCommand.username" class="border rounded-lg bg-white/5 text-sm font-light focus:outline-none focus:border-white/20 border-white/10 px-4 py-2 w-full" placeholder="ایمیل">
    <input type="password" v-model="loginCommand.password" class="border rounded-lg bg-white/5 text-sm font-light focus:outline-none focus:border-white/20 border-white/10 px-4 py-2 w-full" placeholder="رمز عبور">
    <div class="flex items-start gap-2 self-start">
      <base-checkbox v-model="loginCommand.rememberMe" />
      <span>مرا به خاطر بسپار</span>
    </div>
    <a href="" class="font-light text-sm text-indigo-400 underline underline-offset-4 self-start">رمز عبورم را فراموش کرده ام</a>
    <UButton w-full :is-loading="loading" @click="login" class="py-3 rounded-md w-full flex justify-center text-white bg-indigo-600 items-center gap-2 self-end">
      <span>ورود</span>
    </UButton>
    ...
    <button v-if="false" class="py-3 rounded-md w-full flex justify-center text-white border border-[#222E49] items-center gap-2 self-end">
    ...
    <button class="font-light text-sm underline-offset-4 underline" @click="authStore.setAuthState('register')">ساخت حساب کاربری</button>
  </div>
</template>
...
const loginCommand:LoginViewModel = reactive({
  username:'',
  password:'',
  rememberMe:false
})
...
const login = async ()=>{
  ...
  if(result.isSuccess){
    ...
    setTimeout(accountStore.initData,100);
    toast.showToast('با موفقیت وارد شدید');
  }else{
