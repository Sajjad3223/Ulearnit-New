...
<template>
  <li class="flex flex-col space-y-4 py-0.5">
    ...
    <div :class="['p-4 rounded-xl flex flex-col gap-2 cursor-pointer transition-all duration-200',
      ...
      <Transition name="pop">
        <div class="flex items-center gap-2 !origin-right" v-if="showSeason">
      ...
      <div class="flex flex-col space-y-4" v-if="showSeason">
        ...
        <ul class=" bg-[#111723] rounded-xl">
          <li class="flex items-center justify-between p-4 group hover:bg-black/20 transition-colors duration-200"
              v-for="(episode,i) in section.episodes">
            <div class="flex items-start gap-2">
              <base-checkbox/>
              <span>{{ i + 1 }} - {{ episode.title }}</span>
            </div>
            <small class=" mr-auto font-thin opacity-70">{{ episode.time }}</small>
            <button v-if="userHasCourse || episode.isFree"
                    @click="emits('episodeSelected',episode)"
                    class="relative group grid place-items-center p-2 transition-colors duration-200 rounded-lg hover:bg-white/5">
              ...
              <span
                  class="absolute w-max transition-all duration-200 opacity-0 scale-0 translate-y-full group-hover:scale-90 group-hover:translate-y-0 group-hover:opacity-100 -top-2/3 z-20 bg-gray-800 text-xs px-2 py-1 rounded-md pointer-events-none">
                          پخش آنلاین
                        </span>
            </button>
            <button v-else
              ...
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    ...
                    stroke="#F51940" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path fill-rule="evenodd" clip-rule="evenodd"
                      ...
                      stroke="#F51940" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M11.8633 14.2031V16.4241" stroke="#F51940" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round"/>
              </svg>
              <span
                  class="absolute w-max transition-all duration-200 opacity-0 scale-0 translate-y-full group-hover:scale-90 group-hover:translate-y-0 group-hover:opacity-100 -top-2/3 z-20 bg-[#F51940] text-xs px-2 py-1 rounded-md pointer-events-none">
        ...
        <div>
          <div class="flex items-center justify-between p-4 bg-[#060911] h-[100px] ml-8 relative"
               v-for="quiz in section.sectionQuizzes" :key="quiz.id">
            <div class="flex flex-col">
              <strong>{{ quiz.title }}</strong>
              <span class="bg-white/10 rounded-md text-xs px-2 py-1 w-max">{{ quiz.totalQuestions }} سؤال</span>
            </div>
            <button class="bg-[#1CB965]/10 text-[#1CB965] rounded-md px-4 py-2" v-if="quiz.userScore != null">
              <!-- If quiz is passed -->
              قبول شده ( {{ quiz.userScore }} / {{ quiz.passScore }} )
            </button>
            <button class="bg-[#3C42E7] rounded-md px-4 py-2" v-else
                    style="box-shadow: 0 0 10px rgba(60, 66, 231, 0.5);">
              شروع
            </button>
            <div></div>
